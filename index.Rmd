...
In another word, it has "a hole" in the middle, hence the name 
...
- Throughout this presentation until the case study, we will be using an index function called holes index
  - will not show the full formula, but it is proportional to 1 - something that looks like a standardised gaussian density
- on the right I have calculated the index value for some 1D projections and we can observe that ... 
...
???
- In the ferrn package, there are 4 diagnostic plots that we created. Two for exploring the trace and two for space
