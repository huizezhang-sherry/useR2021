...
- In the ferrn package, there are 4 diagnostic plots being created. Two for exploring the trace and two for space
- There is also botanical theme for scale the color
...
Here is the plot made with `explore_space_pca()`.
  - It is the same optimisation problem we have in the previous example
    - where x2 is informative and other 4 variables are random normal
  - First tell you how this plot is made and then what we can learn from it

- We have all the bases evaluated in the optimisation collected in the dataset - these are the coloured dots 
- we also have random bases sampled from the space, these are used to draw the space 
...
we can also make an animated version of this plot 
  - see clearer where each optimiser starts 
